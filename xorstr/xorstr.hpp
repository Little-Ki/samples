...
namespace utility
{
    ...
    constexpr uint32_t initial_seed()
    {
        uint32_t seed = 0;
    ...
    constexpr inline static const uint32_t generate_key(uint32_t& key_seed, const uint32_t multiplier = 48271, const uint32_t modulus = 2147483647, const uint32_t increment = 0)
    {
        key_seed = (key_seed * multiplier + increment) % modulus;
        return static_cast<const uint32_t>(key_seed);
    }
